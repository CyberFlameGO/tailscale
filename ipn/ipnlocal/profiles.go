...
func (pm *ProfileManager) SaveAsStartState(userID string, prefs *ipn.Prefs) error {
	clonedPrefs := prefs.Clone().View()
	pm.mu.Lock()
	defer pm.mu.Unlock()
	if err := pm.switchStateLocked(ipn.StateKey("user-" + userID)); err != nil {
		return err
	}
	if err := pm.setPrefsLocked(clonedPrefs); err != nil {
		return err
	}
...
// elsewhere.
func (pm *ProfileManager) Unload() error {
	pm.mu.Lock()
	defer pm.mu.Unlock()
	pm.prefs = emptyPrefs
	pm.currentState = ""
	if err := pm.useCurrentProfileAtStartup(); err != nil {
		return err
	}
...
// This is a no-op if the current profile is unnamed.
func (pm *ProfileManager) SetPrefs(prefsIn *ipn.Prefs) error {
	clonedPrefs := prefsIn.Clone().View()
	pm.mu.Lock()
	defer pm.mu.Unlock()
	return pm.setPrefsLocked(clonedPrefs)
}
...
func (pm *ProfileManager) loadSavedPrefs(key ipn.StateKey) (ipn.PrefsView, error) {
	...
	case errors.Is(err, ipn.ErrStateNotExist):
		...
		return prefs, nil
	case err != nil:
		return ipn.PrefsView{}, fmt.Errorf("backend prefs: store.ReadState(%q): %v", key, err)
	}
...
func newProfileManagerWithGOOS(store ipn.StateStore, logf logger.Logf, stateKey ipn.StateKey, goos string) (*ProfileManager, error) {
	...
	if stateKey == "" && len(knownProfiles) == 0 && goos != "windows" {
